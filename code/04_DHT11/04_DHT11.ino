 ...
 * กิจกรรมที่ 2.1: DHT11 — วัดอุณหภูมิและความชื้น
 * 
 * สิ่งที่เรียนรู้:
 * - Analog Sensor คืออะไร
 * - การอ่านค่าจากเซนเซอร์ DHT11
 * - Serial Monitor แสดงผลข้อมูล
 * - การใช้ Library
 * 
 * การต่อวงจร:
 * DHT11 Pin 1 (VCC)  → 3.3V
 * DHT11 Pin 2 (DATA) → ESP32 GPIO 14 (ต่อ R 10kΩ pull-up ไป 3.3V)
 * DHT11 Pin 3 (NC)   → ไม่ต่อ
 * DHT11 Pin 4 (GND)  → GND
 * 
...
#define DHTPIN  14        // ขาที่ต่อ DHT11
#define DHTTYPE DHT11     // ชนิดเซนเซอร์
...
void loop() {
  ...
  if (isnan(humidity) || isnan(temperature)) {
    Serial.println("❌ อ่านค่าจาก DHT11 ไม่ได้! ตรวจสอบสายว่าต่อถูกต้อง");
    delay(2000);
    return;
  }
  ...
  delay(2000);  // DHT11 ต้องรออย่างน้อย 1 วินาทีระหว่างการอ่าน
}
